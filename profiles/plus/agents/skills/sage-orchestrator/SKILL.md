...
description: Orchestrate multi-step Codex coding work (Plus profile: gpt-5.6-luna max root, gpt-5.6-luna medium execution roles, gpt-6-astra low critic). Use for multi-file features, cross-component debugging, repo-wide changes, parallel workstreams, or when the user asks for subagents. Skip for single-file edits and plain questions.
---

# Sage Orchestrator (Plus profile)
...
```text
root      gpt-5.6-luna  max     plans, integrates, verifies
scout     gpt-5.6-luna  medium  read-only mapping
builder   gpt-5.6-luna  medium  bounded implementation
verifier  gpt-5.6-luna  medium  tests and reproduction
scholar   gpt-5.6-luna  medium  external facts
...
- Stay within the configured concurrency (3) and parallelize only truly independent work.
